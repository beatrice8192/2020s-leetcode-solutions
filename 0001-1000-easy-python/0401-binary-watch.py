class Solution(object):
    ...
    def readBinaryWatch(self, turnedOn):
        ...
        """
        # If we cheat
        # return self.RESULTS[turnedOn] if turnedOn < 9 else []

        # If we solve it
        result = []
        hourBits = self.countBits(11)
        minuteBits = self.countBits(59)
        # hour can at most have 3 LEDs on
        for hour in range(min(4, turnedOn + 1)):
            minute = turnedOn - hour
            hourCombinations = [i for i in range(12) if hourBits[i] == hour]
            minuteCombinations = [i for i in range(60) if minuteBits[i] == minute]
            result.extend(["%d:%02d" % (h, m) for h in hourCombinations for m in minuteCombinations])
        return result
